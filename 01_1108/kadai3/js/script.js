...
const toDoingTask = () => {
  ...
  task.addEventListener('click',() => {
    ...
    const doingList = [...doing.querySelectorAll('.doing-task')].map
    ((task)=> task.outerHTML);
    ...
    doing.innerHTML = [...doingList,addDoingTask()].reduce((prev,current)=> prev + current);
...
addEventListener('click',()=> inputValue.value === '' ? alert('タスクを入力しろ、バカヤロー')
: addTask());
inputForm();
